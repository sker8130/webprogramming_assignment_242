<!-- footer component  -->
<<<<<<< HEAD

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
</head>

<body>
    <h1>FOOTER</h1>
</body>

</html>
=======
<?php
//footer
?>

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="../style.css" />
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
        href="https://fonts.googleapis.com/css2?family=Arvo:ital,wght@0,400;0,700;1,400;1,700&family=Roboto:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet" />
    <title>Footer restaurant</title>
</head>

<footer>
  <div class="">
    <div class="footer-logo">
      <img src="../images/logo-img-2.png" alt="image logo 2" />
    </div>
    <div class="footer-content">
      <div class="branchs">
        <div class="store" id="store1">
          <div class="address">
            <span class="icon-location"
              ><img src="../images/icon-location.png" alt="icon location"
            /></span>
            <span>1986 Hilltop DriveBorger, TX 79007</span>
          </div>
          <div class="store-info">
            <div class="open-hour">
              <span
                ><img src="../images/icon-watch.png" alt="icon watch"
              /></span>
              <span>7.30AM - 9.30PM</span>
            </div>
            <div class="phone-num">
              <span
                ><img src="../images/icon-phone.png" alt="icon phone"
              /></span>
              <span>+880 1630-225015</span>
            </div>
          </div>
        </div>
        <div class="store" id="store2">
          <div class="address">
            <span class="icon-location"
              ><img src="../images/icon-location.png" alt="icon location"
            /></span>
            <span>4877 Rose AvenueNew Orleans, LA 70112</span>
          </div>
          <div class="store-info">
            <div class="open-hour">
              <span
                ><img src="../images/icon-watch.png" alt="icon watch"
              /></span>
              <span>7.30AM - 9.30PM</span>
            </div>
            <div class="phone-num">
              <span
                ><img src="../images/icon-phone.png" alt="icon phone"
              /></span>
              <span>+880 1630-225016</span>
            </div>
          </div>
        </div>
      </div>

      <div class="contact">
        <span class="footer-contact-us">Contact Us</span>
        <div class="hotline">
          <span><img src="../images/icon-phone.png" alt="icon phone" /></span>
          <span>Hotline: 0387847976</span>
        </div>
        <div class="service">
          <span
            ><img src="../images/icon-service.png" alt="img service"
          /></span>
          <span>Customer service</span>
        </div>
        <div class="media-links">
          <ul>
            <li>
              <a href="#">
                <img src="../images/icon-fb.png" alt="icon facebook" />
              </a>
            </li>
            <li>
              <a href="#">
                <img src="../images/icon-ig.png" alt="icon instagram" />
              </a>
            </li>
            <li>
              <a href="#">
                <img src="../images/icon-x.png" alt="icon x" />
              </a>
            </li>
            <li>
              <a href="#">
                <img src="../images/icon-tt.png" alt="icon tiktok" />
              </a>
            </li>
            <li>
              <a href="#">
                <img src="../images/icon-yt.png" alt="icon youtube" />
              </a>
            </li>
          </ul>
        </div>
      </div>
      <div class="subscribe">
        <div class="subscribe-title">
          <span class="subscribe-title1">Enter email</span>
          <span class="subscribe-title2"
            >to receive promotional information</span
          >
        </div>
        <div class="subscribe-form">
          <input
            type="email"
            class="subscribe-input"
            placeholder="Enter your email here"
          />
          <button type="button" class="btn btn-primary btn-subscribe">
            <span class="btn-label">Subcribe</span>
          </button>
        </div>
      </div>
    </div>
  </div>
  <div class="copyright">
    <span>Copyright © 2025 | sker</span>
  </div>
</footer>
>>>>>>> 4e630e81b19bfc51b5330c2022031b8f4c1a6fe2
